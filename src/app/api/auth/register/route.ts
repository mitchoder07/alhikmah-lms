...
export async function POST(req: NextRequest) {
  ...
  // Matric number is OPTIONAL. Only Al-Bashir students should provide it.
  ...
  if (matricNumber && String(matricNumber).trim()) {
    ...
    if (!/^\d{2}\/\d{2}[A-Z]{3}\d{3,5}$/.test(cleanMatric)) {
      return NextResponse.json({ error: 'Matric number must follow format 20/03ECO002 (year/faculty+department+number). Leave blank if you are not an Al-Bashir student.' }, { status: 400 })
    }

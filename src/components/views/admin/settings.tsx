...
import { Lock, Eye, EyeOff, Loader2, Shield, Mail, Save, AlertTriangle, PenTool, Upload, Trash2 } from 'lucide-react'
...
export function AdminSettings() {
  ...
  /**
   * Client-side white-background removal for scanned signatures.
   * Loads the image into a canvas, reads every pixel, and makes near-white
   * pixels (R > 230, G > 230, B > 230) fully transparent. The result is a PNG
   * (which preserves alpha). This is the "pen ink only" effect — no server-side
   * image processing required, works on Vercel serverless.
   */
  ...
  const handleSignatureUpload = async (file: File) => {
    ...
    try {
      ...
      await refresh()
      toast.success('Signature uploaded — it will appear on transcripts you issue.')
    } catch (e: any) {
  ...
  const handleSignatureRemove = async () => {
    if (!confirm('Remove your signature? Transcripts will fall back to showing just your printed name.')) return
    setSigRemoving(true)
    try {
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <CardDescription className="text-xs">
                ...
                the pen ink remains. Your signature will appear on every transcript you issue,
                above your printed name in the Chief Examiner sign block.
              </CardDescription>
            </div>
          </div>
        </CardHeader>
          ...
          {sigPreview ? (
            <div className="space-y-3">
              ...
              <p className="text-xs text-green-700 bg-green-50 rounded-md p-2">
                ✓ Signature active — it will appear on the next transcript you issue.
              </p>
            </div>
          ) : (
            <div className="space-y-3">
